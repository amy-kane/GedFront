...
const ReceptionnisteLayout = ({ children }) => {
  ...
  const navigateToDashboard = () => {
    console.log("Navigation vers dashboard réceptionniste");
    router.push('/receptionniste/dashboard');
  };

  const navigateToTraitement = () => {
    console.log("Navigation vers traitement");
    router.push('/receptionniste/traitement');
  };
  ...
  const navigateToStatistics = () => {
    ...
    router.push('/receptionniste/statistiques');
  };
          ...
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <h2 className="text-lg font-semibold text-gray-700">
                {pathname?.includes('/dashboard') && 'Tableau de bord'}
                {pathname?.includes('/traitement') && 'Traitement des dossiers'}
                {pathname?.includes('/notifications') && 'Notifications'}
                {pathname?.includes('/statistiques') && 'Statistiques'}
                {!pathname?.includes('/receptionniste/') && 'Réception des dossiers'}
              </h2>
            </div>
              ...
              <div className="flex items-center space-x-3">
                <div className="hidden md:block text-right">
                  <p className="text-sm font-medium text-gray-900">
                    {user ? `${user.prenom} ${user.nom}` : 'Réceptionniste'}
                  </p>
                  <p className="text-xs text-gray-500">
                    {user?.email || 'receptionniste@ged.com'}
                  </p>
                </div>
          ...
          <aside className={`${sidebarCollapsed ? 'w-16' : 'w-64'} bg-white rounded-xl shadow-lg transition-all duration-300 flex flex-col`}>
            ...
            <nav className="flex-1 px-3 py-4 space-y-2 overflow-y-auto">
              ...
              <NavItem 
                icon={<ChartIcon />} 
                text="Statistiques" 
                active={isActive('/receptionniste/statistiques')}
                ...
                <div className="flex items-center">
                  <div className="w-10 h-10 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center mr-3">
                    <span className="text-white font-semibold text-sm">
                      {user ? `${user.prenom?.[0]}${user.nom?.[0]}` : 'R'}
                    </span>
                  </div>
