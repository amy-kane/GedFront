// src/app/coordinateur/statistiques/page.js - Version coordinateur
'use client';
...
export default function StatistiquesCoordinateur() {
  ...
  // Couleurs pour les graphiques (thème vert pour coordinateur)
  const COLORS = {
    SOUMIS: '#fbbf24',      // jaune
    COMPLET: '#10b981',     // vert (thème coordinateur)
    INCOMPLET: '#ef4444',   // rouge
    EN_COURS: '#8b5cf6',    // violet
    APPROUVE: '#059669',    // vert foncé
    REJETE: '#6b7280',      // gris
    MASCULIN: '#10b981',    // vert (thème coordinateur)
    FEMININ: '#ec4899',     // rose
    AUTRE: '#6b7280'        // gris
  };
  ...
  useEffect(() => {
    const fetchStatistiques = async () => {
      ...
      try {
        ...
        console.log('=== DEBUG COORDINATEUR ===');
        ...
        if (user) {
          ...
          // Vérifier que l'utilisateur est bien coordinateur
          if (userData.role !== 'COORDINATEUR') {
            console.log('Utilisateur non autorisé, rôle:', userData.role);
            setError('Accès refusé: vous devez être coordinateur pour accéder à cette page.');
            setLoading(false);
            return;
          }
        }
        ...
        setLoading(false);
        console.log('=== FIN DEBUG COORDINATEUR ===');
  ...
  const exportToExcel = async (data) => {
    ...
    // En-tête du rapport
    csvContent += `Rapport Statistiques GED - Coordinateur\n`;
  ...
  const exportToPDF = async (data) => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Statistiques GED Coordinateur - ${new Date().toLocaleDateString('fr-FR')}</title>
          <style>
        ...
        <body>
          <div class="header">
            <h1>Rapport Statistiques GED - Coordinateur</h1>
          ...
          <div class="footer">
            <p>Rapport généré automatiquement par le système GED - Coordinateur</p>
          </div>
        </body>
      </html>
    `;
  ...
  const exportCandidatsToExcel = async (data) => {
    ...
    // En-tête du rapport
    csvContent += `Liste des Candidats Approuvés - ${data.typeLibelle} (Coordinateur)\n`;
    csvContent += `Date d'export: ${new Date().toLocaleDateString('fr-FR')}\n`;
    csvContent += `Exporté par: ${data.utilisateur.prenom} ${data.utilisateur.nom}\n`;
  ...
  const exportCandidatsToPDF = async (data) => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Liste Candidats Approuvés - ${data.typeLibelle} (Coordinateur)</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 20px; }
            .header { text-align: center; margin-bottom: 30px; border-bottom: 2px solid #10b981; padding-bottom: 20px; }
        ...
        <body>
          <div class="header">
            <h1>Liste des Candidats Approuvés - Coordinateur</h1>
          ...
          <div style="margin-top: 30px; text-align: center; font-size: 12px; color: #666;">
            <p>Document généré automatiquement par le système GED - Coordinateur</p>
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-red-700">
                ...
                <div className="mt-4">
                  ...
                  <button
                    onClick={() => router.push('/coordinateur')}
  ...
  return (
    <div className="max-w-7xl mx-auto mt-8 p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Tableau de bord statistiques - Coordinateur</h1>
