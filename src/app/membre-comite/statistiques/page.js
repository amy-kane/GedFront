// src/app/membre-comite/statistiques/page.js - Version MEMBRE_COMITE
'use client';
...
export default function StatistiquesMembre() {
  ...
  // Couleurs pour les graphiques (thème vert pour membre)
  const COLORS = {
    SOUMIS: '#fbbf24',      // jaune
    COMPLET: '#10b981',     // vert (thème membre)
    INCOMPLET: '#ef4444',   // rouge
    EN_COURS: '#8b5cf6',    // violet
    APPROUVE: '#059669',    // vert foncé
    REJETE: '#6b7280',      // gris
    MASCULIN: '#10b981',    // vert (thème membre)
    FEMININ: '#ec4899',     // rose
    AUTRE: '#6b7280'        // gris
  };
    ...
    const fetchStatistiques = async () => {
      ...
      try {
        ...
        if (user) {
          ...
          if (userData.role !== 'MEMBRE_COMITE') {
            console.log('Utilisateur non autorisé, rôle:', userData.role);
            setError('Accès refusé: vous devez être MEMBRE_COMITE pour accéder à cette page.');
            setLoading(false);
            return;
          }
        }
        ...
        setLoading(false);
        console.log('=== FIN DEBUG MEMBRE_COMITE ===');
  ...
  const exportToExcel = async (data) => {
    ...
    // En-tête du rapport
    csvContent += `Rapport Statistiques GED - MEMBRE_COMITE\n`;
    ...
    link.setAttribute('download', `statistiques_MEMBRE_COMITE_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const exportToPDF = async (data) => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Statistiques GED MEMBRE_COMITE - ${new Date().toLocaleDateString('fr-FR')}</title>
          <style>
        ...
        <body>
          <div class="header">
            <h1>Rapport Statistiques GED - MEMBRE_COMITE</h1>
          ...
          <div class="footer">
            <p>Rapport généré automatiquement par le système GED - MEMBRE_COMITE</p>
          </div>
        </body>
      </html>
    `;
  ...
  const exportToCSV = async (data) => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `statistiques_coordinateur_${new Date().toISOString().split('T')[0]}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const exportCandidatsToExcel = async (data) => {
    ...
    // En-tête du rapport
    csvContent += `Liste des Candidats Approuvés - ${data.typeLibelle} (MEMBRE_COMITE)\n`;
    csvContent += `Date d'export: ${new Date().toLocaleDateString('fr-FR')}\n`;
    csvContent += `Exporté par: ${data.utilisateur.prenom} ${data.utilisateur.nom}\n`;
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `candidats_MEMBRE_COMITE_${data.typeLibelle.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const exportCandidatsToCSV = async (data) => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `candidats_coordinateur_${data.typeLibelle.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const exportCandidatsToPDF = async (data) => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Liste Candidats Approuvés - ${data.typeLibelle} (MEMBRE_COMITE)</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 20px; }
            .header { text-align: center; margin-bottom: 30px; border-bottom: 2px solid #10b981; padding-bottom: 20px; }
            .header h1 { color: #10b981; margin-bottom: 10px; }
            table { width: 100%; border-collapse: collapse; margin: 20px 0; }
            ...
            .sexe-m { background-color: #10b981; }
        ...
        <body>
          <div class="header">
            <h1>Liste des Candidats Approuvés - MEMBRE_COMITE</h1>
            <h2 style="color: #10b981; margin: 10px 0;">${data.typeLibelle}</h2>
            <p>Date d'export: ${new Date().toLocaleDateString('fr-FR')}</p>
            <p>Exporté par: ${data.utilisateur.prenom || ''} ${data.utilisateur.nom || ''}</p>
          </div>
          ...
          <div style="margin-top: 30px; text-align: center; font-size: 12px; color: #666;">
            <p>Document généré automatiquement par le système GED - MEMBRE_COMITE</p>
  ...
  // Composant de carte statistique (thème vert)
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-red-700">
                ...
                <div className="mt-4">
                  ...
                  <button
                    onClick={() => router.push('/membre-comite')}
  ...
  return (
    <div className="max-w-7xl mx-auto mt-8 p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Tableau de bord statistiques - Membre du comité</h1>
              ...
              {Object.entries(statsDemographiques.repartitionSexeParType || {}).map(([typeLabel, sexeStats]) => (
                <div key={typeLabel} className="border-b border-gray-200 pb-4 last:border-b-0">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-green-600">{sexeStats.MASCULIN || 0}</p>
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="w-full bg-gray-200 rounded-full h-3 flex overflow-hidden">
                          <div className="bg-green-500 h-full" style={{ width: `${pctHommes}%` }}></div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <ResponsiveContainer width="100%" height={400}>
              ...
              >
                ...
                <Legend />
                <Bar dataKey="ageMoyen" fill="#10b981" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {/* ✅ BOUTONS D'EXPORT FONCTIONNELS - Thème vert */}
